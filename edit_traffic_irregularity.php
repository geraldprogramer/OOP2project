...
<html lang="en">
  <head>
    ...
    <title>Traffic Irregularities</title>
...
<form action="" method="POST" enctype="multipart/form-data">
        ...
        <br><br>
        <label for="traffic_irregularity">Description of the indiscpline </label><br>
        ...
        <input type="reset">
        <a href="view_traffic_irregularity.php" style="background-color: blue;color: white; padding: 8px 18px; text-align: center; text-decoration: none; border-radius: 5px;";>Cancel</a>
    ...
    if(isset($_POST['update_btn'])){
        ...
        if($image!="")
        {
            $update="UPDATE traffic_irregularity SET names='$names', id_number='$id_number', phone='$phone',  passport_photo='$image', traffic_irregularity='$traffic_irregularity', action_taken='$action_taken' WHERE id='$id'";
            $update_q=mysqli_query($con,$update);
            header('location:view_traffic_irregularity.php');
        }
        else
        {
            $update="UPDATE traffic_irregularity SET names='$names', id_number='$id_number', phone='$phone',traffic_irregularity='$traffic_irregularity',action_taken='$action_taken' WHERE id='$id'";
            $update_q=mysqli_query($con,$update);
            header('location:view_traffic_irregularity.php');
        }
    }
